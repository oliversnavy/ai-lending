...
# Compute c-stat
# For survival data, c-stat = concordance_index(events, scores, times)
# But we have binary events (event = 1 if charged off, 0 otherwise)
# So c-stat = AUC of default_prob vs event

c_stat = roc_auc_score(val['event'], val_probs)
print(f"C-stat (AUC): {c_stat:.4f}")

# Also compute concordance_index from lifelines
try:
    ci = concordance_index(val['observed_time'], val_probs, val['event'])
    print(f"Concordance index (lifelines): {ci:.4f}")
except:
    print("Could not compute concordance index")
