...
# Key parameters
COVERAGE_RATE = 0.16  # Cost of capital
MIN_OFFER_RATE = 0.21  # Minimum viable offer rate
CAPITAL_CAP = 15_000_000
VOLUME_FLOOR = 400
MEDIAN_OBS_TIME = 28  # months

# For each applicant, compute expected P&L per dollar
# E[P&L] = offered_rate * obs_time/12 * (1-default_prob) - default_prob
# But we need to weight by acceptance probability

# Strategy: For each applicant, offer a rate based on their risk profile
# Higher risk = higher rate, but not too high to avoid rejection

# Let me first compute expected P&L at different rate/grade combinations
# using median values for loan_amnt and observed_time

print("Expected P&L per dollar at different rates (using median obs_time=28 months):")
print(f"{'Grade':<6} {'Rate':>6} {'Accept%':>8} {'Default%':>9} {'P&L/$':>8} {'Exp P&L/$':>10}")
print("-" * 50)

for grade in ['C', 'D', 'E', 'F', 'G']:
    subset = val[val['grade'] == grade]
    default_prob = subset['default_prob'].mean()
    
    for rate in [0.21, 0.25, 0.30, 0.35, 0.40]:
        # Get acceptance prob from sensitivity model
        df_test = pd.DataFrame([{
            'grade': grade, 'offered_rate': rate,
            'loan_amnt': 12000, 'annual_inc': 60000, 'funded_amnt': 12000
        }])
        accept_prob = sens_model.predict_proba_batch(df_test)[0]
        
        # Expected P&L per dollar
        # interest = rate * obs_time/12 * (1-default_prob)
        # loss = default_prob
        obs_time_months = 28
        interest_yield = rate * obs_time_months / 12 * (1 - default_prob)
        expected_pnl_per_dollar = interest_yield - default_prob
        
        print(f"{grade:<6} {rate:>6.0%} {accept_prob:>8.2%} {default_prob:>9.4f} {interest_yield:>8.4f} {expected_pnl_per_dollar:>10.4f}")
