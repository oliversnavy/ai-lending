...
# Test more strategies with different parameters
def run_strategy(name, rate_fn, grade_filter=None):
...
# Strategy D: Conservative - only C and D grades
def rate_D(grade, dp):
    if grade not in ['C', 'D']:
        return 0
...
strategies = [
    ('A_base_high', rate_A),
    ('B_very_high', rate_B),
    ('C_market_plus', rate_C),
    ('D_conservative_CD', rate_D),
    ('E_include_G', rate_E),
]

for name, fn in strategies:
    result = run_strategy(name, fn)
