...
# Test different rate strategies
rates_to_test = [0.21, 0.22, 0.23, 0.24, 0.25, 0.26, 0.28, 0.30, 0.35]

print("=== Rate Strategy Comparison ===")
print(f"{'Rate':>6} | {'Loans':>6} | {'Principal':>12} | {'Exp P&L':>12} | {'P&L/$':>8} | {'Accept%':>8}")
print("-" * 60)
...
for rate in rates_to_test:
    ...
    if len(selected) > 0:
        print(f"{rate:>6.0%} | {len(selected):>6} | ${selected['loan_amnt'].sum():>11,.0f} | ${expected_pnl.sum():>11,.0f} | {expected_pnl.sum()/selected['loan_amnt'].sum():>7.3f} | {accept_probs.mean():>7.3f}")
    else:
        print(f"{rate:>6.0%} | {len(selected):>6} | {'N/A':>12} | {'N/A':>12} | {'N/A':>8} | {accept_probs.mean():>7.3f}")