...
# Use the best config from previous run: base=0.20, mult=0.5, upper=0.40
# But let me also try a few more configs with upper=0.40
print("=== Targeted Search (upper=0.40) ===")
results_list = []
...
print(f"\nBest: base={results_df.iloc[0]['base']:.0%}, mult={results_df.iloc[0]['mult']:.1f}")
print(f"Best P&L: ${results_df.iloc[0]['pnl']:,.0f}")