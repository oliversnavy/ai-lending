...
y_pred_gb = model_gb.predict_proba(X_val)[:, 1]
...
val_features['pred_prob'] = y_pred_gb if auc_gb > auc_lr else y_pred_lr
for grade in ['A', 'B', 'C', 'D', 'E', 'F', 'G']:
    mask = val_full['grade'] == grade
    grade_auc = roc_auc_score(val_full.loc[mask, 'event'], val_full.loc[mask, 'pred_prob'])
    print(f"  {grade}: AUC={grade_auc:.4f}, n={mask.sum()}, default_rate={val_full.loc[mask, 'event'].mean():.3f}")
