...
# Since all 1000 applicants are presented, we should offer to all of them (or at least most).
# The capital constraint limits how many we can actually fund.

# Let's try a simple approach: offer a fixed rate to all applicants
# Then select the best ones by expected P&L per dollar

for rate in [0.21, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
    sample['offered_rate'] = rate
    accept_probs = sensitivity_model.predict_proba_batch(sample[['grade', 'offered_rate', 'loan_amnt', 'annual_inc', 'funded_amnt']])
    
    avg_time = sample['observed_time'].mean()
    interest = sample['loan_amnt'] * rate * (avg_time / 12)
    loss = sample['loan_amnt'] * sample['risk_score']
    
    expected_pnl = accept_probs * (interest - loss)
    expected_principal = accept_probs * sample['loan_amnt']
...
# Now let's try a smarter approach: offer different rates based on grade
print("\n=== Grade-specific rates ===")
for c_rate in [0.25, 0.30, 0.35]:
    for d_rate in [0.25, 0.30, 0.35, 0.40]:
        for e_rate in [0.30, 0.35, 0.40, 0.45]:
            sample['offered_rate'] = sample['grade'].map({
                'C': c_rate, 'D': d_rate, 'E': e_rate,
                'F': e_rate + 0.05, 'G': e_rate + 0.10
            }).fillna(0.21)
            
            accept_df = sample[['grade', 'offered_rate', 'loan_amnt', 'annual_inc', 'funded_amnt']].copy()
            accept_probs = sensitivity_model.predict_proba_batch(accept_df)
            
            avg_time = sample['observed_time'].mean()
            interest = sample['loan_amnt'] * sample['offered_rate'] * (avg_time / 12)
            loss = sample['loan_amnt'] * sample['risk_score']
            ...
            if n_loans >= 400:
                print(f"C={c_rate:.0%}, D={d_rate:.0%}, E={e_rate:.0%}: P&L=${total_pnl:,.0f}, N={n_loans}, Accept={accept_rate:.2%}")