...
# === Simulate one episode with 1000 applicants ===
np.random.seed(42)
n_applicants = 1000
sample_idx = np.random.choice(len(val), n_applicants, replace=False)
sample = val.iloc[sample_idx].copy()
sample['risk_score'] = val_risk_score[sample_idx]

sensitivity_model = SensitivityModel()

# Key insight: We need to select which applicants to make offers to, and at what rate.
# Since all 1000 applicants are presented, we should offer to all of them.
# The capital constraint limits how many we can actually fund.
...
sample_np = {
    ...
    'risk_score': sample['risk_score'].values,
    'observed_time': sample['observed_time'].values,
}

print("=== Fixed rate per episode ===")
for rate in [0.21, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55]:
    sample_np['offered_rate'] = np.full(len(sample_np['grade']), rate)
    accept_probs = sensitivity_model.predict_proba_batch(
        pd.DataFrame(sample_np)
    )
    
    avg_time = sample_np['observed_time'].mean()
    interest = sample_np['loan_amnt'] * rate * (avg_time / 12)
    loss = sample_np['loan_amnt'] * sample_np['risk_score']
    
    expected_pnl = accept_probs * (interest - loss)
    expected_principal = accept_probs * sample_np['loan_amnt']
    
    pnl_per_dollar = expected_pnl / expected_principal.clip(lower=1)
    ...
    selected_pnl = expected_pnl[sort_idx[:cap_idx]]
    selected_principal = expected_principal[sort_idx[:cap_idx]]
    
    total_pnl = selected_pnl.sum()
    total_principal = selected_principal.sum()
    accept_rate = accept_probs[sort_idx[:cap_idx]].mean()
    n_loans = len(selected_pnl)
    
    meets_floor = n_loans >= 400
    print(f"rate={rate:.2%}: P&L=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, Accept={accept_rate:.2%}, N={n_loans}, Floor={meets_floor}")

# Now try grade-specific rates
print("\n=== Grade-specific rates (search) ===")
best_pnl = -np.inf
best_config = None

for c_rate in [0.25, 0.30, 0.35, 0.40]:
    for d_rate in [0.25, 0.30, 0.35, 0.40]:
        for e_rate in [0.30, 0.35, 0.40, 0.45, 0.50]:
            for f_rate in [0.35, 0.40, 0.45, 0.50, 0.55]:
                for g_rate in [0.40, 0.45, 0.50, 0.55]:
                    sample_np['offered_rate'] = np.where(
                        sample_np['grade'] == 'C', c_rate,
                        np.where(sample_np['grade'] == 'D', d_rate,
                        np.where(sample_np['grade'] == 'E', e_rate,
                        np.where(sample_np['grade'] == 'F', f_rate,
                        np.where(sample_np['grade'] == 'G', g_rate, 0.21)))))
                    ...
                    avg_time = sample_np['observed_time'].mean()
                    interest = sample_np['loan_amnt'] * sample_np['offered_rate'] * (avg_time / 12)
                    loss = sample_np['loan_amnt'] * sample_np['risk_score']
                    ...
                    if n_loans >= 400 and total_pnl > best_pnl:
                        best_pnl = total_pnl
                        best_config = (c_rate, d_rate, e_rate, f_rate, g_rate)
                        best_accept = accept_probs[sort_idx[:cap_idx]].mean()

print(f"Best: C={best_config[0]:.0%}, D={best_config[1]:.0%}, E={best_config[2]:.0%}, F={best_config[3]:.0%}, G={best_config[4]:.0%}")
print(f"P&L=${best_pnl:,.0f}, Accept={best_accept:.2%}")