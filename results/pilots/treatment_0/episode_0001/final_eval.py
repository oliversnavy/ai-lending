...
# Check fixed 60% rate
print("=== Fixed 60% rate ===")
offered_rates = np.full(len(val), 0.60)
...
print(f"P&L=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")

# Check what happens with no risk model differentiation (just fixed rate)
print("\n=== Fixed rate sweep ===")
for rate in [0.21, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]:
...
# Now let's try the best configuration: base=40%, k=10.0
print("\n=== Best config: base=40%, k=10.0 ===")
base_rate = 0.40
k = 10.0
...
# Print some statistics about the selected loans
selected = val.iloc[sort_idx[:cap_idx]]
print(f"\nSelected loans stats:")
print(f"  Average loan_amnt: ${selected['loan_amnt'].mean():,.0f}")
print(f"  Average annual_inc: ${selected['annual_inc'].mean():,.0f}")
print(f"  Average risk_score: {val_risk_score[sort_idx[:cap_idx]].mean():.4f}")
print(f"  Average offered_rate: {offered_rates[sort_idx[:cap_idx]].mean():.2%}")
