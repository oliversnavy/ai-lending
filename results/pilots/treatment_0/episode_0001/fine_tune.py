...
# Use the best configuration from previous run: base=30%, k=1.0, m=0.2
# But let me also try a few more configurations around that area
print("=== Fine-tuning around best config ===")
grade_rates = {'A': 0.095, 'B': 0.140, 'C': 0.190, 'D': 0.240, 'E': 0.295, 'F': 0.340, 'G': 0.370}
grade_factors = np.array([grade_rates[g] for g in val['grade'].values])
...
for base_rate, k, m in configs:
    offered_rates = np.clip(base_rate + k * val_risk_score + m * grade_factors, 0.21, 0.60)
    ...
    print(f"base={base_rate:.0%}, k={k:.2f}, m={m:.2f}: P&L=${total_pnl:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")
    
    if total_pnl > best_pnl:
        best_pnl = total_pnl
        best_config = (base_rate, k, m)

print(f"\nBest: base={best_config[0]:.0%}, k={best_config[1]:.2f}, m={best_config[2]:.2f}")
print(f"P&L=${best_pnl:,.0f}")

# Now let's also try without the grade factor but with higher k values
print("\n=== Just risk score, higher k ===")
for base_rate in [0.25, 0.30, 0.35]:
    for k in [1.5, 2.0, 2.5, 3.0]:
        offered_rates = np.clip(base_rate + k * val_risk_score, 0.21, 0.60)
        ...
        if total_pnl > best_pnl:
            best_pnl = total_pnl
            best_config = (base_rate, k, 0.0)
            print(f"NEW BEST: base={base_rate:.0%}, k={k:.1f}: P&L=${total_pnl:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")

print(f"\nFinal best: base={best_config[0]:.0%}, k={best_config[1]:.2f}, m={best_config[2]:.2f}")
print(f"Final P&L=${best_pnl:,.0f}")