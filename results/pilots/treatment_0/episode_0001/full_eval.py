...
# Compute risk model concordance index on validation set
event = val['event'].values
observed_time = val['observed_time'].values
c_stat = concordance_index(observed_time, -val_risk_score, event)
print(f"Risk model C-stat: {c_stat:.4f}")

sensitivity_model = SensitivityModel()

# Evaluate on full validation set with different strategies
# Strategy: offer a fixed rate to all, then select by P&L/dollar

print("=== Full validation set evaluation ===")
for rate in [0.21, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
    ...
    print(f"rate={rate:.0%}: P&L=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")

# Now try grade-specific rates on full set
print("\n=== Grade-specific rates on full set ===")
best_pnl = -np.inf
best_config = None

for c_rate in [0.25, 0.30, 0.35]:
    for d_rate in [0.25, 0.30, 0.35]:
        for e_rate in [0.30, 0.35, 0.40]:
            for f_rate in [0.35, 0.40, 0.45]:
                for g_rate in [0.40, 0.45, 0.50]:
                    offered_rates = np.where(
                        val['grade'].values == 'C', c_rate,
                        np.where(val['grade'].values == 'D', d_rate,
                        np.where(val['grade'].values == 'E', e_rate,
                        np.where(val['grade'].values == 'F', f_rate,
                        np.where(val['grade'].values == 'G', g_rate, 0.21)))))
                    ...
                    if n_loans >= 400 and total_pnl > best_pnl:
                        ...
                        best_principal = expected_principal[sort_idx[:cap_idx]].sum()

print(f"Best: C={best_config[0]:.0%}, D={best_config[1]:.0%}, E={best_config[2]:.0%}, F={best_config[3]:.0%}, G={best_config[4]:.0%}")
print(f"P&L=${best_pnl:,.0f}, Principal=${best_principal:,.0f}, Accept={best_accept:.2%}")