...
# Try to find the optimal risk-score + grade-based pricing
print("=== Optimizing risk-score + grade-based pricing ===")
...
for base_rate in np.arange(0.20, 0.40, 0.01):
    for k in [0.5, 0.75, 1.0, 1.25, 1.5]:
        for m in [0.0, 0.05, 0.1, 0.15, 0.2]:
            ...
            if n_loans >= 400 and total_pnl > best_pnl:
                ...
                best_principal = total_principal
                best_n_loans = n_loans

print(f"Best config: base={best_config[0]:.2%}, k={best_config[1]:.2f}, m={best_config[2]:.2f}")
print(f"P&L=${best_pnl:,.0f}, Principal=${best_principal:,.0f}, Accept={best_accept:.2%}, N={best_n_loans}")

# Also try without grade factor, just risk score
print("\n=== Just risk score, wider search ===")
for base_rate in np.arange(0.20, 0.50, 0.01):
