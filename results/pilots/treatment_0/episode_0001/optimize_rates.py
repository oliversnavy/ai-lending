...
# === Optimize rate per grade ===
print("=== Optimizing rate per grade ===")

# For each grade, find the rate that maximizes expected P&L per dollar
for grade in ['C', 'D', 'E', 'F', 'G']:
    mask = val['grade'] == grade
    grade_data = val[mask].copy()
    grade_data['risk_score'] = val_risk_score[mask]
    
    best_pnl_per_dollar = -np.inf
    best_rate = None
    best_accept_prob = None
    
    for rate in np.arange(0.21, 0.55, 0.005):
        test_df = pd.DataFrame({
            'grade': [grade] * len(grade_data),
            'offered_rate': [rate] * len(grade_data),
            'loan_amnt': grade_data['loan_amnt'].values,
            'annual_inc': grade_data['annual_inc'].values,
            'funded_amnt': grade_data['funded_amnt'].values
        })
        accept_prob = sensitivity_model.predict_proba_batch(test_df)
        
        avg_time = grade_data['observed_time'].mean()
        interest = grade_data['loan_amnt'] * rate * (avg_time / 12)
        loss = grade_data['loan_amnt'] * grade_data['risk_score']
        
        pnl = accept_prob * (interest - loss)
        principal = accept_prob * grade_data['loan_amnt']
        
        pnl_per_dollar = (pnl / principal.clip(lower=1)).mean()
        
        if pnl_per_dollar > best_pnl_per_dollar:
            best_pnl_per_dollar = pnl_per_dollar
            best_rate = rate
            best_accept_prob = accept_prob.mean()
    
    print(f"  Grade {grade}: best_rate={best_rate:.2%}, accept={best_accept_prob:.2%}, P&L/$={best_pnl_per_dollar:.4f}")

# Now try combining rates: use the optimal rate for each grade
print("\n=== Combined rates per grade ===")
optimal_rates = {}
for grade in ['C', 'D', 'E', 'F', 'G']:
    mask = val['grade'] == grade
    grade_data = val[mask].copy()
    ...
    for rate in np.arange(0.21, 0.55, 0.005):
        ...
        if pnl_per_dollar > best_pnl_per_dollar:
            ...
            best_rate = rate
            best_accept_prob = accept_prob.mean()
    optimal_rates[grade] = best_rate

print("Optimal rates per grade:")
for grade, rate in optimal_rates.items():
    print(f"  {grade}: {rate:.2%}")

# Apply these rates to the full portfolio
offered_rates = val['grade'].map(optimal_rates).fillna(0.21)
# For grades not in optimal_rates (A, B), use 0.21
offered_rates = np.maximum(offered_rates, 0.21)
...
print(f"Loans Selected: {n_loans}")

# Grade distribution
selected_grades = val['grade'].iloc[sort_idx[:cap_idx]]
print(f"\nGrade distribution:")
print(selected_grades.value_counts(normalize=True))

# P&L by grade
for grade in ['C', 'D', 'E', 'F', 'G']:
    mask = selected_grades == grade
    grade_pnl = expected_pnl[sort_idx[:cap_idx]][mask].sum()
    grade_principal = expected_principal[sort_idx[:cap_idx]][mask].sum()
    grade_accept = accept_probs[sort_idx[:cap_idx]][mask].mean()
    print(f"  {grade}: P&L=${grade_pnl:,.0f}, Principal=${grade_principal:,.0f}, Accept={grade_accept:.2%}")