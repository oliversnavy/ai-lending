...
# Try different k values for pricing
for k in [0.0, 0.1, 0.15, 0.2, 0.3, 0.5]:
    ...
    cap_idx = np.searchsorted(cum_principal, CAPITAL_CAP) + 1
    ...
    print(f"k={k:.2f}: P&L=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")

# Now let's also try a more sophisticated approach: optimize rate per grade
print("\n=== Optimizing rate per grade ===")
for grade in ['C', 'D', 'E', 'F', 'G']:
    mask = val['grade'] == grade
    grade_data = val[mask].copy()
    grade_data['risk_score'] = val_risk_score[mask]
    
    # Get acceptance probs at different rates
    best_pnl = -np.inf
    best_rate = None
    best_accept = None
    
    for rate in np.arange(0.21, 0.55, 0.01):
        ...
        if pnl.sum() > best_pnl:
            best_pnl = pnl.sum()
            best_rate = rate
            best_accept = accept_prob.mean()
    
    print(f"  Grade {grade}: best_rate={best_rate:.2%}, accept={best_accept:.2%}, pnl=${best_pnl:,.0f}")