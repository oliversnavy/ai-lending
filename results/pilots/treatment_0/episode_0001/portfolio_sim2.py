...
# Strategy 1: k multiplier approach
print("=== Strategy 1: k multiplier approach ===")
...
# Strategy 2: Fixed rate per grade
print("\n=== Strategy 2: Fixed rate per grade ===")
for fixed_rate in [0.21, 0.25, 0.30, 0.35, 0.40]:
    offered_rates = np.maximum(fixed_rate, val['grade'].map(sensitivity_model.market_rates).values + 0.05)
    offered_rates = np.minimum(offered_rates, 0.50)
    ...
    print(f"rate={fixed_rate:.2f}: P&L=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, Accept={accept_rate:.2%}, N={n_loans}")