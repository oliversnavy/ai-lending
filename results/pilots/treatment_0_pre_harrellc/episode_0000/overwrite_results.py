...
results = {
    ...
    "approach": "Trained a logistic regression default risk model (C-index 0.68) on 466k training loans. Designed grade-based pricing (C:21%, D:24%, E:29.5%, F:34%) with risk-adjustment and burden adjustment. Used sensitivity model for acceptance probabilities and greedily selected highest-ROI loans under $15M cap, yielding a Grade E/F focused portfolio with 52% acceptance rate and 705 expected funded loans.",
    "hypothesis": "Higher-grade (E/F) borrowers with rates near market would accept at higher rates while generating strong P&L due to high interest margins relative to expected losses."
}

with open('results.json', 'w') as f:
    json.dump(results, f, indent=2)

print("Overwrote results.json with current episode results")
