...
# ============================================================
# FINAL STRATEGY: Risk-based pricing with 0.20 multiplier
# ============================================================

# Only viable grades C-F (A/B won't accept at 21%+)
viable_grades = ['C', 'D', 'E', 'F']
val_v = val[val['grade'].isin(viable_grades)].copy()

# Risk-based pricing: rate = 0.21 + risk_score * 0.20, clamped to [0.21, 0.36]
...
# Greedy selection by P&L per dollar
...
print(f"P&L per dollar deployed: {total_pnl / total_principal:.4f}")
print(f"Average offer rate: {sel['offered_rate'].mean():.4f}")
print(f"Average risk score: {sel['risk_score'].mean():.4f}")

# Grade breakdown
print(f"\nPortfolio grade breakdown:")
for grade in ['C', 'D', 'E', 'F']:
    g = sel[sel['grade'] == grade]
    print(f"  Grade {grade}: {len(g)} loans, principal=${g['expected_principal'].sum():,.0f}, "
          f"P&L=${g['expected_pnl'].sum():,.0f}, acceptance={g['p_accept'].mean():.3f}")

# Risk score distribution
print(f"\nRisk score distribution:")
print(sel['risk_score'].describe())
...
results = {
    ...
    "approach": "Risk-based pricing: offered rate = 21% + risk_score * 20%, clamped to [21%, 36%]. Focused on Grade C-F borrowers where market rates exceed our 21% cost floor. Used gradient boosted tree risk model (AUC=0.695) trained on LendingClub training data. Greedy selection by expected P&L per dollar deployed under $15M capital cap.",
