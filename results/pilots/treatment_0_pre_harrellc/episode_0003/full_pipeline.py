...
print(f"Viable: {len(val_viable)}")

# --- Optimized pricing: parametric approach ---
# Instead of grid search per applicant, use a formula:
# rate = min(0.36, max(0.21, base_rate + risk_adjustment))
# 
# From grid search analysis:
# - Base rates by grade: C=0.28, D=0.31, E=0.34, F=0.34, G=0.34
# - Risk adjustment: +0.5 * default_prob (higher risk → higher rate)
# - Also adjust for loan/income ratio (burden)

# Let's compute a parametric rate for each applicant
# Then use batch prediction to get acceptance probabilities
...
# Sort by expected_pnl / expected_principal for greedy selection
...
results_json = {
    ...
    "approach": "Logistic regression risk model scores applicants; parametric pricing maps grade + default probability to optimal rate in 21-36% band; Grade C-F targeted with greedy capital allocation",
    "hypothesis": "Optimal rates around 27-34% for Grade C-F borrowers maximize expected P&L by balancing acceptance probability against interest income and default losses"
}
