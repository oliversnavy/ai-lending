"""
Final optimization: risk-based pricing with fine grid search + capital cap.
"""
...
# ============================================================
# Fine grid search for risk-based pricing
# ============================================================
...
for base_mult in np.arange(15, 50, 2):
...
print(f"\nTop 5 strategies:")
sorted_results = sorted(best_results, key=lambda x: x['pnl'], reverse=True)
for r in sorted_results[:5]:
    print(f"  mult={r['mult']:.1f}: PnL=${r['pnl']:,.0f}, Principal=${r['principal']:,.0f}, "
...
results = {
    ...
    "approach": f"Logistic regression default model (C-index={lr_ci:.4f}) with risk-based pricing; offer rate = base_rate + (default_prob - 0.18)*15 * multiplier, optimized over fine grid of multipliers (15-50) with $15M capital cap and capital-efficient loan selection",
    "hypothesis": "Risk-based pricing with fine-grained multiplier optimization maximizes P&L by balancing acceptance probability against expected default losses within the 21-36% rate band"
}
