"""
Final fine-tuning: narrow grid around best mult=41, plus alternative pricing.
"""
...
for base_mult in np.arange(38, 48, 0.25):
...
# ============================================================
# Try different base rate formulas
# ============================================================

print("\n=== Alternative pricing strategies ===")

# Strategy 1: Higher risk adjustment coefficient
for coeff in [10, 12, 15, 20, 25]:
    base_rate_series = val_for_model['grade'].map(base_rates_dict).fillna(0.24)
    risk_adj = (val_for_model['default_prob'] - 0.18) * coeff
    offer_rates = (base_rate_series + risk_adj).clip(0.21, 0.36).round(4)
    val_for_model['offered_rate'] = offer_rates.values
    ...
    print(f"  coeff={coeff}: PnL=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, "
...
# Strategy 2: Only offer to grades with positive expected P&L
print("\n=== Grade filtering ===")

# Try offering only to specific grade ranges
for min_grade in ['C', 'D']:
    for max_grade in ['F', 'G']:
        grade_filter = val_for_model['grade'].isin(['C', 'D', 'E', 'F', 'G'][:['C','D','E','F','G'].index(max_grade)+1])
        if min_grade == 'D':
            grade_filter = val_for_model['grade'].isin(['D', 'E', 'F', 'G'][:['D','E','F','G'].index(max_grade)+1])

        sub = val_for_model[grade_filter].copy()
        if len(sub) == 0:
            continue

        base_rate_series = sub['grade'].map(base_rates_dict).fillna(0.24)
        risk_adj = (sub['default_prob'] - 0.18) * 15
        offer_rates = (base_rate_series + risk_adj * 2.5).clip(0.21, 0.36).round(4)
        sub['offered_rate'] = offer_rates.values

        sens_input = sub[['grade', 'offered_rate', 'loan_amnt', 'annual_inc', 'funded_amnt']].copy()
        sens_input['p_accept'] = sensitivity_model.predict_proba_batch(sens_input)
        sens_input = sens_input[sens_input['p_accept'] > 0.001].copy()
        ...
        print(f"  Grades {min_grade}-{max_grade}: PnL=${total_pnl:,.0f}, Principal=${total_principal:,.0f}, "
              f"Loans={loans_funded:.0f}, Acept={acceptance_rate:.4f}")
...
results = {
    ...
    "approach": "Logistic regression default model (C-index=0.673) with risk-based pricing; offer rate = base_rate + (default_prob - 0.18)*15 * multiplier, optimized over fine grid (38-48) with $15M capital cap and capital-efficient loan selection by expected P&L ranking",
