"""
Quick optimization: gradient boosting + per-grade rate optimization.
"""
...
gb_pred = gb.predict_proba(val_X_scaled)[:, 1]
gb_ci = concordance_index(val_df['observed_time'], -gb_pred, val_df['event'])
print(f"GB C-index: {gb_ci:.4f}")

# Train LR model for comparison
...
# Blend
blend_pred = 0.5 * gb_pred + 0.5 * lr_pred
...
print(f"Using {model_name} model (C-index={final_ci:.4f})")

# ============================================================
# Per-grade rate optimization using sensitivity model
# ============================================================
...
# For each grade, try different rates and find the one that maximizes P&L
best_grade_rates = {}
for grade in ['C', 'D', 'E', 'F', 'G']:
    ...
    for rate_cents in np.arange(21.0, 36.5, 0.25):
...
# Now apply the optimized rates to the full dataset
print("\n=== Applying optimized rates ===")

grade_rates = pd.Series(best_grade_rates)
# Default rate for grades not in the dict
grade_rates = grade_rates.reindex(['A','B','C','D','E','F','G'], fill_value=0.21)
...
# Also try risk-based pricing (default prob → rate)
print("\n=== Risk-based pricing strategies ===")

best_pnl = total_pnl
best_data = sens_input.copy()
best_params = {'type': 'grade_opt', 'ci': final_ci}

for base_mult in [5, 10, 15, 20, 25, 30, 40, 50]:
    base_rates = {
    ...
    if total_pnl > best_pnl:
        best_pnl = total_pnl
        best_data = sens_input.copy()
        best_params = {'type': 'risk_mult', 'mult': base_mult, 'ci': final_ci}

print(f"\nBest: {best_params}, PnL=${best_pnl:,.0f}")
...
print(f"C-index: {final_ci:.4f}")

results = {
    "pnl": float(final_pnl),
    "c_stat": float(final_ci),
    "acceptance_rate": float(final_acceptance),
    "loans_funded": final_loans,
    "total_principal": float(final_principal),
    "approach": f"{model_name} model (C-index={final_ci:.4f}) with grade-optimized pricing; rates 21-36% tuned per grade to maximize expected P&L via sensitivity model",
